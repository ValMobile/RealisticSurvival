...
public class EndermanLookForPlayerGoal_v1_21_R4 extends NearestAttackableTargetGoal<Player> {
    ...
    public boolean canContinueToUse() {
        if (pendingTarget != null) {
            if (v1_21_R4.isLookingAtMe(enderman, pendingTarget)) {
    ...
    public void tick() {
        ...
        if (pendingTarget != null) {
            ...
            if (target != null && !enderman.isPassenger()) {
                if (v1_21_R4.isLookingAtMe(enderman, (Player) target)) {
                    if (target.distanceToSqr(enderman) < 16.0) {
                        enderman.teleport();
                    }
                    teleportTime = 0;
                }
                else if (target.distanceToSqr(enderman) > 256.0 && teleportTime++ >= adjustedTickDelay(30) && v1_21_R4.teleportTowards(enderman, target)) {
                    teleportTime = 0;
                }
            }
            super.tick();
