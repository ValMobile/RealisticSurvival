...
public class FollowOwnerGoal_v26_1_R3 extends Goal {
    ...
    public FollowOwnerGoal_v26_1_R3(RSVPet pet, double d0, float f, float f1, boolean flag) {
        this.pet = pet;
    ...
    private boolean maybeTeleportTo(int i, int j, int k) {
        ...
        if (Math.abs((double)i - owner.getX()) < 2.0 && Math.abs((double)k - owner.getZ()) < 2.0) {
            ...
            if (event.isCancelled()) {
                ...
                to = event.getTo();
                mob.snapTo(to.getX(), to.getY(), to.getZ(), to.getYaw(), to.getPitch());
                navigation.stop();
                return true;
            }
        }
