...
public class EndermanLookForPlayerGoal_v1_20_R7 extends NearestAttackableTargetGoal<Player> {
    ...
    public EndermanLookForPlayerGoal_v1_20_R7(EnderMan enderman, @Nullable Predicate<LivingEntity> predicate) {
        super(enderman, Player.class, 10, false, false, predicate);
        this.enderman = enderman;
        this.startAggroTargetConditions = TargetingConditions.forCombat().range(getFollowDistance()).selector((entityliving) -> v1_20_R7.isLookingAtMe(enderman, (Player) entityliving));
    }
    ...
    public boolean canContinueToUse() {
        if (pendingTarget != null) {
            if (v1_20_R7.isLookingAtMe(enderman, pendingTarget)) {
    ...
    public void tick() {
        ...
        if (pendingTarget != null) {
            ...
            if (target != null && !enderman.isPassenger()) {
                ...
                else if (target.distanceToSqr(enderman) > 256.0 && teleportTime++ >= adjustedTickDelay(30) && v1_20_R7.teleportTowards(enderman, target)) {
                    teleportTime = 0;
                }
            }
            super.tick();
