...
public abstract class ExplosionAttack_v1_16_R1 extends BukkitRunnable {

    private final Utils util = RealisticSurvivalPlugin.getUtil();
    private final Dragon_v1_16_R1 dragon;
    ...
    public ExplosionAttack_v1_16_R1(Dragon_v1_16_R1 dragon, Location target, RealisticSurvivalPlugin plugin) {
        ...
        double magnitude = config.getInt("Dragons." + capitalizeBreed + "Dragon.ExplosionAttack.Velocity");

        this.plugin = plugin;
        this.loc = e.getLocation();
        this.world = loc.getWorld();
        this.velocity = loc.toVector().subtract(target.toVector()).normalize().multiply(magnitude * stage);
        this.radius = config.getInt("Dragons." + capitalizeBreed + "Dragon.ExplosionAttack.Radius") * stage;
        this.block = world.spawnFallingBlock(loc, Material.valueOf(config.getString("Dragons." + capitalizeBreed + "Dragon.ExplosionAttack.FallingBlock")).createBlockData());
        this.particle = Particle.valueOf(config.getString("Dragons." + capitalizeBreed + "Dragon.ExplosionAttack.Particle"));
        this.decayTicks = config.getInt("Dragons." + capitalizeBreed + "Dragon.ExplosionAttack.DespawnTime");
    ...
    public final void run() {
        ...
        // summon a particle
        world.spawnParticle(particle, loc, 5, 0.5, 0.5, 0.5, 0, null, true);
