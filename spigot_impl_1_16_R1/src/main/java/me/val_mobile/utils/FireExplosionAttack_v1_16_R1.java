...
public class FireExplosionAttack_v1_16_R1 extends ExplosionAttack_v1_16_R1 {

    private final FireDragon_v1_16_R1 dragon;

    public FireExplosionAttack_v1_16_R1(FireDragon_v1_16_R1 dragon, Location target, RealisticSurvivalPlugin plugin) {
                ...
                for (int z = -radius; z < radius; z++) {
                    ...
                    if (!(Math.hypot(Math.hypot(x, y), z) > radius)) {
                        ...
                        // if the upper block is solid
                            ...
                            if (!config.getStringList("Dragons.FireDragon.ExplosionAttack.ImmuneBlocks").contains(blockMaterial.toString())) {
                                // if the upper block is solid
                                if (!block.isLiquid()) {
                                    burnt = config.getConfigurationSection("Dragons.FireDragon.ExplosionAttack.SpecialBlocks").getKeys(false).contains(blockMaterial.toString())
                                            ? Material.valueOf(config.getString("Dragon.FireDragon.ExplosionAttack.SpecialBlocks." + blockMaterial))
                                            : (blockMaterial.isFuel()
                                            ? Material.valueOf(config.getString("Dragons.FireDragon.ExplosionAttack.BurntBlock"))
